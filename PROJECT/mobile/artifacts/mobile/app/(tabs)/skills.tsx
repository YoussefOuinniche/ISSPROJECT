...
import { analyzeSkillGapsWithAi, generateRoadmapWithAi } from "@/lib/api/mobileApi";
...
export default function SkillsScreen() {
  ...
  const generateRoadmap = async () => {
    const role = effectiveTargetRole?.trim();
    if (!role) {
      setRoadmapError("Set a target role in Complete Your Profile before generating a roadmap.");
      return;
    }
    setRoadmapGenerating(true);
    setRoadmapError(null);
    try {
      await generateRoadmapWithAi({ role });
      await Promise.allSettled([refreshProfile(), queryClient.invalidateQueries()]);
      router.push({
        pathname: "/learn/[id]",
        params: {
  ...
  return (
    ...
    >
      ...
      <GlassCard style={styles.roadmapGenCard} padding={20} radius={20}>
        <View style={styles.roadmapGenRow}>
          <View style={styles.roadmapGenIcon}>
            <Feather name="map" size={20} color={Colors.primary} />
          </View>
          <View style={styles.roadmapGenCopy}>
            <Text style={styles.roadmapGenTitle}>
              {roadmap?.role ? "Regenerate Roadmap" : "Generate AI Roadmap"}
            </Text>
            <Text style={styles.roadmapGenSub}>
              {effectiveTargetRole
                ? `Build a step-by-step learning plan for ${effectiveTargetRole}`
                : "Add a target role to generate your personalised roadmap"}
            </Text>
          </View>
        </View>
        <Pressable
          style={[styles.roadmapGenBtn, (!effectiveTargetRole || roadmapGenerating) && styles.roadmapGenBtnDisabled]}
          disabled={!effectiveTargetRole || roadmapGenerating}
        ...
        >
          ...
          <Text style={styles.roadmapGenBtnText}>
            {roadmapGenerating ? "Generating…" : roadmap?.role ? "Regenerate" : "Generate Roadmap"}
          </Text>
        </Pressable>
        {roadmapError ? <Text style={styles.roadmapGenError}>{roadmapError}</Text> : null}
      </GlassCard>
...
const styles = StyleSheet.create({
  ...
  roadmapGenCard: { marginBottom: 18 },
  roadmapGenRow: { flexDirection: "row", alignItems: "flex-start", gap: 14, marginBottom: 16 },
  roadmapGenIcon: {
    width: 46, height: 46, borderRadius: 14,
    backgroundColor: Colors.primary + "15",
    alignItems: "center", justifyContent: "center",
  },
  roadmapGenCopy: { flex: 1 },
  roadmapGenTitle: { fontSize: 16, fontFamily: "Inter_700Bold", color: Colors.textPrimary },
  roadmapGenSub: {
    marginTop: 4, fontSize: 13, lineHeight: 19,
    fontFamily: "Inter_400Regular", color: Colors.textSecondary,
  },
  roadmapGenBtn: {
    height: 46, borderRadius: 13, backgroundColor: Colors.primary,
    flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8,
  },
  roadmapGenBtnDisabled: { opacity: 0.5 },
  roadmapGenBtnText: { fontSize: 14, fontFamily: "Inter_700Bold", color: "#FFFFFF" },
  roadmapGenError: {
    marginTop: 10, fontSize: 12, fontFamily: "Inter_500Medium", color: Colors.danger,
  },
});
