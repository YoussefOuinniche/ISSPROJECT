...
router.get("/user/home", async (req, res) => {
  const upstream = await proxyLegacy(req, "/user/home", "GET");
  return res.status(upstream.status).json(upstream.data);
});

router.get("/user/ai/history", async (req, res) => {
  const upstream = await proxyLegacy(req, "/user/ai/history", "GET");
  return res.status(upstream.status).json(upstream.data);
});

router.post("/user/ai/chat", async (req, res) => {
  const upstream = await proxyLegacy(req, "/user/ai/chat", "POST", req.body);
  return res.status(upstream.status).json(upstream.data);
});

router.get("/user/skills", async (req, res) => {
  const upstream = await proxyLegacy(req, "/user/skills", "GET");
  return res.status(upstream.status).json(upstream.data);
});

router.post("/user/skills", async (req, res) => {
  const upstream = await proxyLegacy(req, "/user/skills", "POST", req.body);
  return res.status(upstream.status).json(upstream.data);
});

router.put("/user/skills/:skillId", async (req, res) => {
  const upstream = await proxyLegacy(req, `/user/skills/${req.params.skillId}`, "PUT", req.body);
  return res.status(upstream.status).json(upstream.data);
});

router.delete("/user/skills/:skillId", async (req, res) => {
  const upstream = await proxyLegacy(req, `/user/skills/${req.params.skillId}`, "DELETE");
  return res.status(upstream.status).json(upstream.data);
});
