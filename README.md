...
A learner creates an account or signs in.
The app checks whether a valid session already exists and routes the user automatically.
If the learner has no roadmap yet, the app guides them through an AI onboarding assessment.
The assessment collects profile information, target role, current skills, education, experience, weekly learning availability, and knowledge scores.
The roadmap generator analyzes skill gaps, maps those gaps to learning resources, asks the local LLM for a structured learning plan, then saves the roadmap and steps.
The learner follows the roadmap, marks steps as started or completed, and can share a completed roadmap with the community.
The learner can use AI chat, recommendations, job search, trends, and profile insights to keep the plan aligned with their career goal.
Repository Layout
PROJECT/
...
app.json / Expo extra.apiBaseUrl, if provided.
Backend URL derived from the Expo development host IP.
Cached working backend URL from AsyncStorage.
http://localhost:5000.
